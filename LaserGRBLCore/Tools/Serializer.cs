	...
	public class Serializer
	{
		...
		public static bool ObjToFile(object ObjectToSave, string filename, SerializationMode SerializeMode, string Password, bool Compression)
		{
			lock (ThreadLock)
			{
				...
				try
				{
					filename = System.IO.Path.Combine(LaserGRBLCore.GrblCore.DataPath, filename);
		...
		private class LogDatedWriter
		{
			...
			{
				LaserGRBLCore.Logger.LogMessage("Serialization", Text);
			}
			...
			{
				WriteLogLines("");
                LaserGRBLCore.Logger.LogMessage("Serialization", "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
			}
